...
def skipgram_embedding(data):
    ...
    try:
        ...
        model = gensim.models.Word2Vec(corpus, min_count = 1, size = 100, window = 5, sg = 1)
...
if __name__ == '__main__':
    ...
    train_A_skipgram = skipgram_embedding(train_A)
    write_dict_to_json(train_A_skipgram, 'train_A_skipgram.json.gz')
    
    train_B_skipgram = skipgram_embedding(train_B)
    write_dict_to_json(train_B_skipgram, 'train_B_skipgram.json.gz')
    
    test_A_skipgram = skipgram_embedding(test_A)
    write_dict_to_json(test_A_skipgram, 'test_A_skipgram.json.gz')
    
    test_B_skipgram = skipgram_embedding(test_B)
    write_dict_to_json(test_B_skipgram, 'test_B_skipgram.json.gz')